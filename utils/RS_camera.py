# Realsense camera related classes and functions
...
class CameraRS:
	def __init__(self, device, cc):
		...
		if cc['enable_color']:
			rs_config.enable_stream(rs.stream.color, cc['width'], cc['height'], color_format , cc['fps'])
		...
		if cc['record_bag']:
			print('recording')
			rs_config.enable_record_to_file(os.path.join(cc['save_path'],
														 f'camera_{self.serial_number}.bag'))
		##########################
		### Start the pipeline ###
		##########################
		...
		self.color_sensor.set_option(rs.option.exposure, 39)
		...
		##################################
		if cc['save_camera_para'] and cc['enable_depth'] and cc['enable_color']:
